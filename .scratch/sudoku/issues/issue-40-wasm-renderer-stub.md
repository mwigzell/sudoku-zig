...
`main.zig` reads a CLI arg to choose between ASCII/ANSI and WASM modes. Default: `-r/--renderer ansi` preserving current behaviour. The flag only affects what happens, not how the binary compiles — same executable runs both paths when compiled natively.
