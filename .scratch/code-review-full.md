...
**Status:** Fixed in commits `c2855f3`, `2ea5f6f` — removed custom clones, replaced with stdlib `std.mem.count(u8, ...)`.
...
**Status:** Fixed in commit `88c4437` — renamed to `DIM_ON` and updated all call sites.
...
|------|----------|-------------|
| **Standards** | 4 (2 fixed, 2 captured as issues) | ~~S2~~ — `BOLD_ON` lied. Renamed to `DIM_ON`. S1 removed in favor of stdlib `std.mem.count`. |
