...
- **Commands**: JS sends player actions as **string arguments** to exported functions (e.g., `fill_cell("3 5 7")`). Format is free-form per command; parsing happens inside Zig. This matches exactly how the terminal receives input (`stdin → GameEngine.parse_command`), so no new parsing layer exists for browser.
...
- Matches our terminal command parsing path — no duplication of intent-extraction logic for browser
